...
class App(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.disciplines_data = load_game_data('data/disciplines.json')
        ...
        self.selected_discipline_for_2_dots = tk.StringVar(value=None)
        ...
        ctk.CTkFrame(self.scrollable_frame, height=2, fg_color="gray30").pack(fill='x', padx=20, pady=20)
        self._create_disciplines_section()
        ctk.CTkFrame(self.scrollable_frame, height=2, fg_color="gray30").pack(fill='x', padx=20, pady=20)
    ...
    def _create_status_trackers_section(self):
        ...
        trackers_main_frame.pack(fill="x", padx=20, pady=5) # Reduzido o pady para diminuir o espaço vertical
        ...
        # grid para colocar os medidores lado a lado
        columns_container = ctk.CTkFrame(trackers_main_frame, fg_color="transparent")
        columns_container.pack(fill="x", expand=True, pady=(5, 0)) # Reduzido o pady
        
        # configura o grid para ter duas colunas de tamanho igual que se expandem
        columns_container.grid_columnconfigure((0, 1), weight=1)
        
        # frame da bitalidade (Coluna 0)
        health_frame = ctk.CTkFrame(columns_container)
        health_frame.grid(row=0, column=0, padx=(0, 5), pady=5, sticky="nsew") # padx para dar espaço entre colunas
        
        ctk.CTkLabel(health_frame, text="Vitalidade", font=self.normal_font).pack(pady=(5, 0))
        self.health_tracker_container = ctk.CTkFrame(health_frame, fg_color="transparent")
        self.health_tracker_container.pack(pady=5, padx=5, expand=True) # expand=True para ajudar na centralização

        # frame da Força de Vontade (Coluna 1)
        willpower_frame = ctk.CTkFrame(columns_container)
        willpower_frame.grid(row=0, column=1, padx=(5, 0), pady=5, sticky="nsew") # padx para dar espaço entre colunas
        
        ctk.CTkLabel(willpower_frame, text="Força de Vontade", font=self.normal_font).pack(pady=(5, 0))
        self.willpower_tracker_container = ctk.CTkFrame(willpower_frame, fg_color="transparent")
        self.willpower_tracker_container.pack(pady=5, padx=5, expand=True) # expand=True para ajudar na centralização
    ...
    def _create_disciplines_section(self):
        #CTkFrame normal, não um LabelFrame
        disciplines_frame = ctk.CTkFrame(self.scrollable_frame, fg_color="transparent")
        disciplines_frame.pack(fill="x", expand=True, padx=20, pady=10)
        
        self._populate_disciplines_frame(disciplines_frame)
    ...
    def _create_tracker_row(self, container, tracker_type, size):
        ...
        for i in range(size):
            button = ctk.CTkButton(container, text="", width=28, height=28, font=self.tracker_font, fg_color="#343638", border_width=2, border_color="gray50", hover_color="gray25", command=lambda t=tracker_type, idx=i: self._on_tracker_click(t, idx))
    ...
    def _populate_disciplines_frame(self, parent_frame):
        """Cria a interface para a seleção de Disciplinas."""
        ctk.CTkLabel(parent_frame, text="Escolha suas Disciplinas de Clã", font=self.category_font).pack(pady=5)
        
        instruction_text = (
            "Seu personagem recebe 2 pontos em uma Disciplina de clã e 1 ponto em outra.\n"
            "Selecione qual Disciplina receberá 2 pontos."
        )
        ctk.CTkLabel(parent_frame, text=instruction_text, wraplength=400).pack(pady=(0, 15))
        
        # Frame para conter os widgets das disciplinas
        self.disciplines_container = ctk.CTkFrame(parent_frame, fg_color="transparent")
    ...
    def _update_disciplines_display(self):
        """Limpa e redesenha a seleção de disciplinas com base no clã escolhido."""
        # Limpa widgets antigos
        for widget in self.disciplines_container.winfo_children():
            widget.destroy()
        ...
        for discipline_name in clan_disciplines:
            row_frame = ctk.CTkFrame(self.disciplines_container, fg_color="transparent")
            row_frame.pack(fill="x", padx=150, pady=5, anchor="center")

            selector = ctk.CTkRadioButton(
                row_frame,
                text=discipline_name,
                variable=self.selected_discipline_for_2_dots,
